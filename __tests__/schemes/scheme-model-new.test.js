...
describe("Schemes model", () => {
    ...
    describe("findbyId()", () => {
        ...
        test(`Throws an error if called with no argument`, async () => {
            expect.assertions(1);
            try {
                await schemeModel.findById();
            }
            catch (e) {
                expect(e.toString()).toMatch(/ersror/i);
            }
        })
    })
    ...
    describe("findByName", () => {
        ...
        test(`Throws an error if called with no argument`, async () => {
            expect.assertions(1);
            try {
                await schemeModel.findByName();
            }
            catch (e) {
                expect(e.toString()).toMatch(/error/i);
            }
        })
    })
    ...
    describe("add() a scheme", () => {
        ...
        test("Throws an error if there are extra properties", async () => {
            expect.assertions(1);
            ...
            try {
                ...
                expect(err.toString()).toMatch(/error/i);
            }
        })
    })
    ...
    describe("update() a scheme", () => {
        ...
        test("Throws an error if name already exists", async () => {
            ...
            try {
                await schemeModel.update(updatedScheme, 1);
            }
            catch (e) {
                expect(e.toString()).toMatch(/error/i);
            }
    ...
    describe("findStepById()", () => {
        ...
        test(`Throws an error if called with no argument`, async () => {
            expect.assertions(1);
            try {
                await schemeModel.findStepById();
            }
            catch (e) {
                expect(e.toString()).toMatch(/error/i);
            }
